...
class MusicRepository {
    ...
    suspend fun scanMount(cfg: MountConfig): Int {
        ...
        // Carry over anything the user set (favorite, play count, cached state,
        // original "date added") for tracks that already existed before this rescan,
        // instead of blowing them away just because we re-list the share.
        val existing = db.trackDao().observeByMount(cfg.id).first()
            .associateBy { it.id }
        val now = System.currentTimeMillis()
        val merged = tracks.map { t ->
            val prev = existing[t.id]
            t.toEntity().let { fresh ->
                if (prev == null) fresh.copy(addedAt = now)
                else fresh.copy(
    ...
    suspend fun scanLocal(): Int {
        ...
        val existing = db.trackDao().observeLocal().first().associateBy { it.id }
        val now = System.currentTimeMillis()
        val merged = tracks.map { t ->
            val prev = existing[t.id]
            t.toEntity().let { fresh ->
    ...
    private fun Track.toEntity() = TrackEntity(
        ...
        cached = cached, cachePath = cachePath,
        albumArtUri = albumArtUri, isFavorite = isFavorite,
        year = year, addedAt = if (addedAt > 0) addedAt else System.currentTimeMillis(),
    )
